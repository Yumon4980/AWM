...
#### 窗口标识
身份锚定在 **(进程名, 窗口标题)** 上：进程名跨会话稳定，标题用来区分同进程的多个窗口。
HWND 重启即变，不能用于持久化。

| 场景 | 记什么 | 原因 |
|---|---|---|
| 自动折叠的组 | 只记 `Processes`（`Members = null`） | 语义就是"这个进程的全部窗口"，不需要区分 |
| 手工建的分组 | 记 `Members: [{进程, 标题}]` | 必须精确到窗口，否则拖一个 VS Code 窗口进分组会把全部 VS Code 窗口都拽进来 |
| 解散后的窗口 | 每个槽位一条 `Members` | 同上，且这样每个槽位的名字/顺序才能独立保存 |

标题匹配不上时（VS Code 换了文件）**退回按进程认领一个窗口**，
宁可位置不理想，也不能因为标题变化就让窗口掉出分组、看起来"少了一个"。

> ⚠️ **踩过的坑（并组把同进程窗口全拽进来 / 重命名丢失）**：
> 早期只按 `Processes` 记成员，一个进程开多个窗口时就会失控——
> 拖一个 VS Code 窗口进分组，`Processes: [Code.exe]` 会认领**全部** 5 个窗口。
> 重命名丢失则是 `ToDocument` 把同进程的多个槽位**合并成一条定义**，
> 而合并后的定义只能带一个 `Name`，于是重命名的名字被丢掉；
> 而且一条定义同样会去抢该进程的全部窗口。
> 现在每个槽位写一条带 `Members` 的定义，名字和窗口归属都独立保留。
> `ResolvedSlot.WithName` 也必须带上 `Members`——漏了它就会静默退化成"整个进程的窗口"。
...
**二级（组内）只允许排序，禁止嵌套分组**：分区退化为上下各半，中间不再有"并入"区，
`Ctrl` 在组内也不生效。层级严格保持两级。

组内顺序写进 `SlotDefinition.MemberOrder`（窗口标题列表）。
这是**提示**不是硬绑定——标题变了（VS Code 换文件）匹配不上的成员退回 z-order 排在最后，
绝不因为标题变化就把窗口藏起来。只有用户真的在组内拖过才写这个字段。

判定逻辑抽在 `DropHitTest`（纯函数），因为这块埋进 WPF 事件里就没法测了。

> ⚠️ **踩过的坑 1（拖动高概率没反应）**：ghost 窗口是 `Topmost` 且正好压在光标底下，
> 会抢走鼠标消息，`SelectorWindow` 于是收不到 `MouseMove`，落点判定一直停在拖动开始那一刻
> （通常就是源行本身，于是"什么都没发生"）。
> 两层修复：① ghost 打上 `WS_EX_TRANSPARENT | WS_EX_NOACTIVATE` 真正点击穿透
> （WPF 的 `IsHitTestVisible` 只管窗口**内部**命中测试，管不了 Win32 谁收消息）；
> ② 落点判定改由 16ms 的跟随定时器驱动，直接读 `Cursor.Position`，不依赖鼠标事件是否送达。
>
> ⚠️ **踩过的坑 2（缝隙被漏判）**：`ListBoxItem` 之间有 `Margin="0,1"`。
> 原来用"落在行的上下边界之间"判定，缝里的像素两边都不匹配，于是掉进"追加到末尾"的兜底分支——
> 表现为拖动莫名跑到最后。改成"第一个底边在光标之下的行"，缝自然归到下一行顶部。
> 单测里逐像素扫过整个列表验证了 0 个像素被漏判。
...
> ⚠️ **踩过的坑（右键菜单无效）**，两个独立原因：
>
> 1. **不能用 WPF 的 `ContextMenu`。** 它是独立的弹出窗口，而选择器是 `Topmost`，
>    弹出层很容易被主窗口盖在后面（用户看到的就是"右键没反应"）；
>    它在 `ControlTemplate` 里靠 `PlacementTarget + RelativeSource` 接 DataContext 也不够可靠。
>    改成**画在自己窗口的可视树里**（`PART_Menu`，`PART_Root` 的兄弟，用 `Margin` 定位），
>    层级、激活权、绑定上下文的问题一次性全没了。
> 2. **模态对话框会让 owner 失焦**（已实测确认）。`RenameDialog.ShowDialog()` 触发
>    `SelectorWindow.Deactivated` → "失焦即关闭"把选择器关了 → 对话框成孤儿 → 看起来就是"改名没反应"。
>    修法是 `_suppressAutoClose` 守卫：菜单/对话框期间不响应失焦自动关闭，
>    对话框关掉后再 `Activate()` 把焦点抢回来。
>
> ⚠️ **重命名时索引键会被全局钩子吞掉。** 钩子在选择器显示期间会吃掉 16 个索引键，
> 于是输入框里打 `123qwe` 一个字都进不去。修法是 `SuspendIndexCaptureChanged` 事件——
> 对话框打开前请求 App 放行索引键，关掉后恢复。搜索模式复用同一个开关。

#### 解散分组
右键组 → 解散。拆成"每个窗口一个槽位"，每个槽位记 `Members = [{进程, 标题}]`。
因为记的是**具体那个窗口**而不是整个进程，所以不会在下次解析时又被折叠回去——
这正是"解散分组点了没反应"的原因。
