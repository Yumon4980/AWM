...
- **锁定槽位**：锁定后固定键位；程序关闭后槽位保留为"未运行"占位，按键重新启动
...
每个格子右上角有把锁：点击**锁定**后，这个槽位固定占住当前键位，拖动 / 排序 / 删除都不会移动它。

锁定还带来一个关键能力——**程序关了，槽位还在**：

- 锁定的**程序**窗口被关闭后，格子里保留一个压暗的"未运行"占位；按这个键位（或选中后 `Enter`）会用锁定时记录的可执行路径**重新启动**程序。
- 锁定的**程序组**：成员程序关掉后组内同样保留"未运行"占位，进入二级按对应成员键逐个拉起；成员全部关闭后组也不会消失。
- **程序组合**：按下时已开的激活、未开的自动启动（记录了每个成员的可执行路径）。
- 锁定 / 解锁随时可切；解锁后窗口一关，槽位照旧消失。

> 可执行路径在**点击锁定那一刻**从运行中的进程反查并写入 `layout.json`。
> 旧版本锁定的槽位没有记录路径，如需"重新打开"能力，解锁后重新锁定一次即可。
