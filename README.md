...
| `1 2 3 4` / `Q W E R` / `A S D F` / `Z X C V` | 16 个槽位。命中**程序**直接切；命中**程序组**进入二级；命中**程序组合**打开组合内全部程序 |
| `Esc` / `Backspace` | **逐级回退**：二级 → 一级 → 关闭。在组里按 Esc 等于撤销刚才那次按键 |
| `Tab` / `↓` | 选中下一项（到底循环） |
| `Shift + Tab` / `↑` | 选中上一项 |
| `Enter` | 确认。程序组进组，程序组合打开全部，程序切过去 |
| `/` | 进入搜索模式（跨组扁平搜索），`Esc` 退出 |
| 鼠标拖动 | 拖到**行与行之间**=排序；拖到**行中部**=目标是程序/程序组合则建**程序组合**，目标是程序组则**加入该组** |
| 鼠标右键 | 重命名程序/程序组/程序组合（留空恢复默认名）、**关闭程序**（关掉后选择器保持打开，可连续关）、删除程序组、**解散程序组合**（一级二级都支持）、移出分组 |
| 左上角 `+ 新建程序组` | 新建一个空程序组（会先问名字），再把程序/程序组合拖到它那一行上即可加入 |
...
|---|---|
| **程序** | 一个窗口。按下直接切过去 |
| **程序组** | 容器。按下进入二级，里面装**程序**和**程序组合**（严格两级，程序组不能再嵌套） |
| **程序组合** | 一组程序（最多 4 个）。按下**同时/依次打开全部**：已开则激活，未开则用记录的可执行路径启动 |

### 分组 / 组合

- 同一进程开了 ≥2 个窗口时**自动折叠成一个程序组**，只占一个槽位（`AutoGroupThreshold` 可调，设 99 等于关闭）
- 把一行拖到另一行**中部**：目标是程序/程序组合 → 建**程序组合**（最多 4 个程序，超了不加入）；目标是程序组 → 把这一行**加进该组**
- 拖到**行间**只是排序；按住 `Shift` 强制排序、按住 `Ctrl` 强制并入
- 自动折叠的程序组只剩 1 个成员时自动解散；手工建的程序组即使为空也会保留
- **进入程序组后也能拖动排序 / 并成程序组合**（层级严格两级，组里不能再放程序组）
- 在二级把成员拖到顶部的面包屑上 = **移出该组**
- 槽位超过 16 个时，第 16 格自动变成「更多…」组装下余量
- 托盘菜单有「重置分组与顺序」
- 右键程序组 → **删除程序组**：只删容器，组内成员摊回一级
- 右键任意一行可以**重命名**，留空则恢复默认名

> **身份锚定在「进程名 + 窗口标题」上**。
> 手工建的程序组 / 程序组合记的是**具体哪些窗口**——所以把一个 VS Code 窗口拖进组，
> 不会把其余 VS Code 窗口也一起拽进来。
> 自动折叠出来的组则按进程记（语义本来就是"这个程序的全部窗口"）。
> 标题变了（VS Code 换文件）时退回按进程匹配，不会让窗口掉出分组。
>
> 程序组合还会记住每个程序的**可执行路径**，窗口关掉后再按组合就能重新拉起来。
